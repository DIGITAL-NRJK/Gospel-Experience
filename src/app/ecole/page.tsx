  ...
  return (
    <>
      ...
      <section className="relative min-h-[350px] md:min-h-[400px] bg-gradient-to-br from-[#0D3D2A] to-[var(--color-indigo)] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[rgba(13,61,42,0.8)] to-[rgba(43,27,94,0.85)]" />
        <div className="site-container relative z-10 py-12 md:py-16">
          <div className="max-w-[520px]">
            <div className="text-[12px] tracking-[3px] uppercase text-[var(--color-teal)] font-bold mb-3">
            ...
            <p className="text-[16px] text-white/65 leading-relaxed mb-7">
              {s?.ecoleHeroSubtitle || "Un dimanche par mois, rejoignez notre atelier chœur gospel au Carré Fourvière."}
            </p>
      ...
      <section className="py-12 md:py-16">
        <div className="site-container">
          <div className="section-tag text-[var(--color-teal)]">{s?.ecoleFormatTag || "Format"}</div>
          <h2 className="font-serif text-[24px] md:text-[30px] font-bold text-[var(--color-indigo)] mb-6">
      ...
      <section className="py-12 md:py-16 bg-gradient-to-b from-[var(--color-cream)] to-[#E8F8F0]">
        <div className="site-container">
          <div className="section-tag text-[var(--color-teal-dark)]">{intervenants.length > 1 ? "Intervenants" : "Intervenant(e)"}</div>
          <h2 className="font-serif text-[24px] md:text-[30px] font-bold text-[var(--color-indigo)] mb-6">
          ...
          <div className={`grid gap-8 ${intervenants.length > 1 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 md:grid-cols-[200px_1fr]"}`}>
            {intervenants.map((p, i) => (
              <div key={i} className={intervenants.length > 1 ? "bg-white rounded-[20px] p-6 border border-[rgba(43,27,94,0.06)]" : "contents"}>
                {/* Photo */}
                <div className={`bg-gradient-to-br from-[var(--color-teal)] to-[#5DCAA5] rounded-[20px] overflow-hidden ${intervenants.length > 1 ? "h-[200px] mb-4" : "min-h-[240px]"}`}>
                  {p.photo && (
                ...
                <div>
                  {intervenants.length > 1 && (
                    <h3 className="font-serif text-[20px] font-bold text-[var(--color-indigo)] mb-1">{p.name}</h3>
                  )}
                  {p.role && (
                    <div className="text-[14px] text-[var(--color-teal)] font-bold mb-2">{p.role}</div>
                  )}
                  {p.bio && (
      ...
      <section id="dates" className="py-12 md:py-16">
        <div className="site-container">
          ...
          <h2 className="font-serif text-[24px] md:text-[30px] font-bold text-[var(--color-indigo)] mb-5">
          ...
          {dates.length > 0 ? (
                ...
                return (
                  <div key={d.date} className="bg-white rounded-2xl px-5 py-4 border border-[rgba(43,27,94,0.06)] flex items-center gap-4">
                    <div className="bg-[var(--color-teal-light)] rounded-xl px-4 py-2.5 text-center">
                      <div className="font-serif text-[22px] font-bold text-[var(--color-teal-dark)] leading-none">{day}</div>
                      <div className="text-[11px] tracking-[1px] uppercase text-[var(--color-teal)]">{monthShort}</div>
                    </div>
                    <div>
                      <div className="text-[15px] font-bold text-[var(--color-indigo)]">Dimanche {day} {monthLong}</div>
            ...
            <p className="text-[15px] text-[var(--color-text-muted)] text-center py-8">Les dates seront annoncées prochainement. Ajoutez-les depuis le Studio → Réglages → Page École GEI.</p>
          )}
        </div>
      </section>
      ...
      <div id="inscription" className="site-container pb-10">
        <div className="bg-[var(--color-teal-light)] rounded-3xl px-6 md:px-8 py-9 text-center">
          <h3 className="font-serif text-[24px] md:text-[28px] font-bold text-[var(--color-teal-dark)] mb-2">
            {s?.ecoleCtaTitle || "Prêt à rejoindre le chœur ?"}
          </h3>
          <p className="text-[15px] text-[var(--color-teal-dark)] opacity-70 mb-5">
