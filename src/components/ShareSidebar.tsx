...
export default function ShareSidebar({ title, slug }: ShareSidebarProps) {
  ...
  return (
    <div className="hidden lg:flex flex-col gap-3 sticky top-32">
      <div className="font-display text-[11px] tracking-[2px] uppercase text-[var(--color-text-light)] mb-1">Partager</div>
      {shareLinks.map((link) => (
        <a
          key={link.label}
          href={link.href}
          ...
          className="w-10 h-10 rounded-full bg-white border border-[rgba(30,21,53,0.08)] flex items-center justify-center text-[var(--color-text-muted)] no-underline hover:bg-[var(--color-brand)] hover:text-white hover:border-[var(--color-brand)] transition-colors"
        >
          {link.icon}
        </a>
      ))}
      <button
        ...
        className={`w-10 h-10 rounded-full border flex items-center justify-center transition-colors cursor-pointer ${
          copied
            ...
            : "bg-white border-[rgba(30,21,53,0.08)] text-[var(--color-text-muted)] hover:bg-[var(--color-brand)] hover:text-white hover:border-[var(--color-brand)]"
        }`}
      >
        {copied ? (
